...
outdir = defs.OUT_PATH+'/test/w_select'
    ...
    for idx in avg_score[avg_score > 2.8*(defs.horizon-1)/defs.N].index:
      ...
      while k < n_episodes:
        ...
        for timestep in range(1, defs.horizon):
          ...
          e_greedy_action = kernel.e_greedy(state, weights, epsilon)
          reward = kernel.get_reward(e_greedy_action, KN_idx)
          state_prime, KN_lc, contaminant_lcs, obs = kernel.next_state(KN_lc, KN_idx, contaminants, contaminant_lcs, contaminant_idx, timestep+1, e_greedy_action)
          R_tau += reward
