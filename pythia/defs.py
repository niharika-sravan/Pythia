...
DATA_PATH = pkg_resources.resource_filename('pythia', 'data')
OUT_PATH = pkg_resources.resource_filename('pythia', 'outdir')

class train_data():
  def __init__(self, status):
    self.KN_lc = pd.read_csv(DATA_PATH+'/KNe_'+status+'.csv')
    self.contaminant_lc = pd.read_csv(DATA_PATH+'/contaminant_'+status+'.csv')
    #remove SBO
...
KN_lc_all = pd.read_csv(DATA_PATH+'/KNe_all.csv')
contaminant_lc_all = pd.read_csv(DATA_PATH+'/contaminant_all.csv')
...
lim1_ToO = joblib.load(DATA_PATH+'/lims_ToO_300_1.joblib')
lim2_ToO = joblib.load(DATA_PATH+'/lims_ToO_300_2.joblib')
