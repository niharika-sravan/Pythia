...
from pythia import utils
from pythia import kernel
from pythia import defs

dest = defs.OUT_PATH+'/n9'
...
while k < n_episodes:
  ...
  for use in range(defs.reuse): #each episode reused, epsilon decayed after
    ...
    for timestep in range(1, defs.horizon): #FIXME: in SARSA action prime does not exist, can we just return reward then?
      state = kernel.State(KN_lc, KN_idx, contaminants, contaminant_lcs, contaminant_idx, timestep)
      e_greedy_action = kernel.e_greedy(state, w, epsilon)
      reward = kernel.get_reward(e_greedy_action, KN_idx)
      state_prime, KN_lc, contaminant_lcs, obs = kernel.next_state(KN_lc, KN_idx,
                                                        contaminants, contaminant_lcs, contaminant_idx,
                                                        timestep+1, e_greedy_action)
      e_greedy_action_prime = kernel.e_greedy(state_prime, w, epsilon)

      if timestep != defs.horizon-1:
        td0 = reward + gamma * kernel.lin_VFA(state_prime, e_greedy_action_prime, w)[0,0]
      else:
        td0 = reward
      delw = (td0 - kernel.lin_VFA(state, e_greedy_action, w)[0,0]) * kernel.feat_state_action(state, e_greedy_action)
