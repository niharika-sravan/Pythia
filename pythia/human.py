...
def main(args=None):
    ...
    while (k < n_episodes or train):
        ...
        for timestep in range(1, defs.horizon): #in SARSA not useful if action prime does not exist
            ...
            reward = kernel.get_reward(action, KN_idx)
            R_tau += reward
            state_prime, KN_lc, contaminant_lcs, obs = kernel.next_state(KN_lc, KN_idx, contaminants, contaminant_lcs, contaminant_idx, timestep+1, action, agent=agent)
