...
while k < n_episodes:
  ...
  for use in range(defs.reuse): #each episode reused, epsilon decayed after
    ...
    for timestep in range(1, defs.horizon-1): #in SARSA not useful if action prime does not exist
      ...
      state_prime, KN_lc, contaminant_lcs, obs = pythia.next_state(KN_lc, KN_idx, contaminants, contaminant_lcs, contaminant_idx,
                                                        timestep+1, e_greedy_action)
      e_greedy_action_prime = pythia.e_greedy(state_prime, w, epsilon)

      delw = (reward +
                gamma * pythia.lin_VFA(state_prime, e_greedy_action_prime, w)[0,0] -
                pythia.lin_VFA(state, e_greedy_action, w)[0,0]
             ) * pythia.feat_state_action(state, e_greedy_action)
  ...
  if epsilon < 0.05 and train_status.groupby('k').mean()['R_tau'].rolling(window=50).mean().iloc[-1] > 2.5*(defs.horizon-2)/defs.N:
